...
public class EPC {
	...
	public static EPC loadEPML(String fileLocation, String epcId){
        ...
        DefaultHandler handler = new EPCParser(result, epcId);
	...
	public String toString(){
		...
		for (Function f: functions.values()){
			result += f.getName() + ", " +f.getType()+ "\n";
		}
		for (Event e: events.values()){
			result += e.getName() + ", " +e.getType()+ "\n";
		}
		for (Connector c: connectors.values()){
			result += c.getName() + ", " +c.getType()+ "\n";
		}
		for (Arc a: arcs){
			result += "(" + a.getSource().getName() + ", " + a.getTarget().getName() + ", " +a.getType()+ ")\n";
		}
